...
Here we visually inspect that the pressure timeseries of each stationary period (1) is correctly grouped and (2) does not include pressure outliers (e.g., altitudinal movements) using the generic `plot()`. It is worth to zoom-in and pan on each individual stationary period manually to check this.
...
::: {.alert .alert-warning role="alert"}
<h4 class="alert-heading mt-2"><strong>How short can be a stationary period?</strong></h4>

It can be common and ok to have a few stationary period/flight shorter than the warning threshold of check 1 and 2 (default can be overwritten `warning_flight_duration = 2`, `warning_stap_duration = 6`, `warning_pressure_diff = 3`, `warning_stap_length = 12`).

However, the number of likelihood map and, more importantly, the size of the graph can become very large, in particular if these maps are not informative, and in between long flight as all pixels will needs to be included in the graph.

This is where the parameter `include_min_duration` from `tag_set_map()` is particularly useful: it allows to ignore the short stationary periods and merge flights. 
...
The shiny app GeoPressureViz is another important tool to be used instead of or in parallel to check 4. The main adventages of using the apps are

- Quickly test the pressurepath of different position for a single stationary period using "Start editing" and "query pressure".
- Separate the likelihood of pressure and light to see where they disagree
...
Labelling should finally be checked at the end of the workflow using the most-likely path. 
