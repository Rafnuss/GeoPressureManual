...
2. Set the **movement model** which define how likely are each possible flight encoded in the graph.
3. **Compute three products** of the trajectory model: (1) the shortest path (i.e., the most likely trajectory), (2) the probability maps at each stationary period (i.e., marginals), and (3) simulated paths sampled according to their likelihood. 


## Background on movement modeling

If you haven't heard of [Hidden Markov model](https://en.wikipedia.org/wiki/Hidden_Markov_model), [states-space models](https://doi.org/10.1016/j.tree.2007.10.009), or [trellis graphs](https://en.wikipedia.org/wiki/Trellis_(graph)), I recommend that you start by reading the following section. An even best suggestion would be to read Patterson et al. ([2008](https://doi.org/10.1016/j.tree.2007.10.009) and Nussbaumer et al. (<a href="https://doi.org/10.1111/2041-210X.14082" target="_blank">2023b</a>) if you feel the courage!
...
This discretication allows us to use a [Hidden Markov model](https://en.wikipedia.org/wiki/Hidden_Markov_model), which conveniently distinguish the **unknown positions** of a bird $X_t$ from some **known observations** $Y_t$ which can be related to these positions (e.g., pressure or light measurement). Defining an HMM requires that we can compute:
...
2. An **movement model** $P(X_t+1|X_t)$ which defines the probability that a bird was able to fly a specific distance between two consecutive stationary periods. 
3. An **initial state** $P(X_0)$, technically required but this does not constitute a strong constraint in our case. 

Let say that our goal is to produce the probability map for each stationary period which accounts for all pressure measurements while maintain likely flights. This corresponds to the **marginal** probability map  $P(X_t=x_t|Y_0=y_0,...Y_t=y_t)$. If you are not familiar with the lower and capital case, $X_t$ represents the random variable (i.e., the position of the bird at a stationary period $t$), while $x_t$ is one specific value that $X$ can take, that is, the specific coordinates of pixels on our map.

Because the number of possible positions is limited (i.e.,  number of pixels on the map), we can solve the HHM exactly. This is in contrast to Markov Chain Monte Carlo (MCMC) algorithm, which does not assume any discretization of the space but relies on an iterative approach to solve the model. 

A graph is a convenient mathematical representation of a model which has states (nodes) and some relations between these states (edges). Graphs are a general and fundamental mathematical tool, but here we use a specific graph structure corresponding to the HMM presented above: two states $X$ and $Y$ for each time step $t$ (nodes), and an observation and movement models (relating $X_t$ to $X_t+1$ and $Y_t$ to $X_t$ respectively). All other relationships are assumed to be independent.

Typically in animal movement, the graph is only used to represent the random variable of the HMM, but in our case, we also use the concept of graph to store all discrete positions (i.e., pixels) of the bird at all timesteps, a [so-called trellis graph](https://en.wikipedia.org/wiki/Trellis_(graph)). 

You can think of the nodes as a full 3D grid of lat x lon x stationary period and the edges as all possible pairs of nodes between two consecutive stationary periods. As you can imagine, this results in a very high number of edges! The novelty of Nussbaumer et al. (<a href="https://doi.org/10.1111/2041-210X.14082" target="_blank">2023b</a>) is to construct this graph using only the useful nodes and edges, and thus to dramatically reduce the computational resources required.
:::
</details>
...
2. Keep the nodes which are within reach of at least one node of the previous and one of the next stationary period. This is done by assuming a maximal possible groundspeed `thr_gs`. This key step is efficiently computed with the image binary distance function `EBImage::distmap()`.
3. Build the edges based on the filtered nodes and the groundspeed threshold `thr_gs`.
4. Prune the graph based on the flow graph constraint, that is, the all nodes must be connected to the equipment node(s) and retrieval node(s). Indeed, after filtering, some nodes might be connected in a single direction (so called dead branch).
...
```{r 03-graph-create, cache = TRUE, warning = F, message=FALSE}
...
With the graph built, we can create three products which are presented in more details in [section 2.3 of Nussbaumer et al. (2023b)](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.14082#mee314082-sec-0010-title)
...
```{r 03-graph-marginal, message=FALSE}
