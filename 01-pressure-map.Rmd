...
[`pam_read()`](https://raphaelnussbaumer.com/GeoPressureR/reference/pam_read.html) currently support data from Swiss Ornithological Institute files (`*.pressure`, `*.lux`, `*.acceleration`) and Migrate Technology (`.deg`, `*.lux`). For other file format, please contact me ([submit a Github issue](https://github.com/Rafnuss/GeoPressureR/issues/new) or [email](mailto:rafnuss@gmail.com)).

## Label tracks

To ensure the high level of precision needed for the pressure match, we must manually edit the activity classification and  the pressure timeseries to be matched. We suggest doing this with TRAINSET. [Labelling tracks] is dedicated to this exercise and includes tips and best practices. Here, we will just walk through the main step of the workflow.
...
Use [`trainset_write()`](https://raphaelnussbaumer.com/GeoPressureR/reference/trainset_write.html) to export the automatically generated classifications in a `csv` file, which can be opened in TRAINSET: [https://trainset.geocene.com/](https://trainset.geocene.com/).
...
![*Printscreen of the manual classification in TRAINSET. See [Labelling tracks] for more information.*](assets/labelling-tracks-0.png){width=100%}

When you have finished the manual editing, export the new `csv` file (TRAINSET will add `-labeled` in the name) in `/data/1_pressure/labels/`) and read this file with [`trainset_read()`](https://raphaelnussbaumer.com/GeoPressureR/reference/trainset_read.html).
...
Based on the activity labeling, [`pam_sta()`](https://raphaelnussbaumer.com/GeoPressureR/reference/pam_sta.html) creates a table of stationary periods as illustrated below.
...
## Compute mistmatch maps {#computing-mistmatch-maps}
...
We can now query the data on the API with [`geopressure_map()`](https://raphaelnussbaumer.com/GeoPressureR/reference/geopressure_map.html). A detailed description of the parameters can be found [here](https://raphaelnussbaumer.com/GeoPressureAPI/#request). This will take a couple of minutes to run.
...
[`geopressure_map()`](https://raphaelnussbaumer.com/GeoPressureR/reference/geopressure_map.html) returns a list of two rasters for each stationary periods. The first is the mean square error ($\textbf{MSE}$) between the pressure timeseries and ERA5 map. The second ($\textbf{z}_{thr}$) is the proportion of datapoints in the pressure timeseries which correspond to an altitude that falls between the min and max altitude of each grid cell. Read more about these values and how they are computed [here](https://raphaelnussbaumer.com/GeoPressureAPI/#description).
...
We first determine the position of the bird by using the most likely postion using [`geopressure_map2path`](https://raphaelnussbaumer.com/GeoPressureR/reference/geopressure_map2path.html)
```{r}
pt <- geopressure_map2path(pressure_prob[1])
```

And then call the function [`geopressure_ts()`](https://raphaelnussbaumer.com/GeoPressureR/reference/geopressure_ts.html) with the subset of pressure containing `sta_id==1`
...
Secondly, we can use [`geopressure_ts_path()`](https://raphaelnussbaumer.com/GeoPressureR/reference/geopressure_ts_path.html) which basically call [`geopressure_ts()`](https://raphaelnussbaumer.com/GeoPressureR/reference/geopressure_ts.html) in parallel for all stationary periods. We can additionally request to compute the altitude during the next flight flight with `include_flight = c(0,1)`. Note that if a position of the path is over water, it will be moved to the closest point onshore.
