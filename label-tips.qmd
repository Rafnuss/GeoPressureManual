...
During a flight, single acceleration measurements can be low due to e.g., gliding, typical of the end of night. Yet, the bird was still flying and this should be included as flight duration and excluded from a stationary period.
...
![*Mis-labeled `flight` on acceleration data during low-activity flight typical to end of night.*](assets/labelling-tracks-1.png){width="100%" alt="Activity plot showing mislabeled flight data during low-activity gliding typical at end of night migration"}
...
![Default view of pressure timeseries showing breeding period at mine site](assets/labelling-tracks-3.png){width="100%" alt="Pressure timeseries plot showing default view where altitude changes are not immediately obvious"}
...
![Zoomed view revealing altitude change at end of breeding period](assets/labelling-tracks-4.png){width="100%" alt="Zoomed pressure timeseries plot clearly showing sudden pressure drop indicating altitude change"}
...
![Edited timeseries with altitude-change data removed](assets/labelling-tracks-5.png){width="100%" alt="Pressure timeseries after removing datapoints during altitude change, showing only consistent elevation period"}
...
![Extended daytime migration with unclear flight ending](assets/labelling-tracks-6.png){width="100%" alt="Activity plot showing extended migration into day with reduced intensity, making flight end unclear"}
...
![Short migration halt showing several hours of low activity](assets/labelling-tracks-7.png){width="100%" alt="Activity timeseries showing bird stopping for hours with ambiguous activity patterns"}
...
![Pressure timeseries analysis for determining stationary periods during halts](assets/labelling-tracks-8.png){width="100%" alt="Pressure plot used to assess whether low activity periods represent true stationary periods or local movements"}
...
![Ring Ouzel pressure timeseries showing daily altitudinal movements](assets/labelling-tracks-9.png){width="100%" alt="Complex pressure timeseries for Ring Ouzel showing daily altitudinal changes with unclear temporal vs spatial variation"} At this point it can help to zoom out on the time axis to see whether a certain elevation seems most common. Then proceed iteratively to keep only the datapoints at the same elevation. Test #4 is useful to ensure you did not forget any points.

![Zoomed out view to identify most common elevation](assets/labelling-tracks-10.png){width="100%" alt="Wider time view of pressure data to identify the most frequently used elevation level"}

![Iterative editing to keep consistent elevation datapoints](assets/labelling-tracks-11.png){width="100%" alt="Results of iterative editing process to retain only datapoints at consistent elevation"}
...
![Eurasian Hoopoe continuous daily movement pattern](assets/labelling-tracks-12.png){width="100%" alt="Sinusoidal pressure pattern showing continuous daily movement of Eurasian Hoopoe"} This is the most challenging case as distinguishing temporal variation from altitudinal change is difficult.

![Complex pattern requiring careful temporal vs altitudinal distinction](assets/labelling-tracks-13.png){width="100%" alt="Challenging pressure timeseries requiring careful distinction between temporal variation and altitude changes"} Several iterations should lead to a relatively smooth pressure timeseries. Note that in order to estimate the uncertainty correctly for such cases, the standard deviation `s` should be increased. Thankfully, this behaviour is restricted to its breeding ground.

![Final smoothed timeseries after multiple iterations](assets/labelling-tracks-14.png){width="100%" alt="Smoothed pressure timeseries after iterative editing with increased uncertainty for complex mountainous behavior"}
...
![Impossible case requiring complete timeseries discard](assets/labelling-tracks-15.png){width="100%" alt="Ring Ouzel wintering site showing no consistent elevation, requiring complete timeseries discard"}
...
![Geographic constraint showing Atlas mountains match for low pressure](assets/labelling-tracks-16.png){width="50%" alt="Map showing Atlas mountains of Morocco as only geographic area matching observed low pressure values"}
