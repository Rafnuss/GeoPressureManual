...
3.  The "Minimum duration" input allows you to filter out short stationary periods on the map, similarly to `include_min_duration` in `tag_set_map()`.
...
9.  Finally, you can also check the pressure timeseries with the new positions using the button "Query pressure". This runs `geopressure_timeseries()` in the background, so it is slow (especially for long stationary periods).

## Run GeoPressureViz

The visualization can be started with your track using `geopressureviz()`.

```{r 07-geopressureviz-run-from-objects}
#| eval: false
include
load_interim("18LX")
geopressureviz(
...
```{r 07-geopressureviz-run-from-id}
#| eval: false
geopressureviz("18LX")
```
