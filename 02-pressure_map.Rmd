...
This second chapter of the basic tutorial covers the main steps to determine the position of a bird from pressure data. This code is a direct implementation of the method introduced in Nussbaumer et al. (<a href="https://doi.org/10.1111/2041-210X.14043" target="_blank">2023a</a>).
...
This value should ideally be calibrated, but it is usually safe to start with a value of `1`. We see how to adjust this value in [check #4 of the labelling procedure](https://raphaelnussbaumer.com/GeoPressureManual/labelling-tracks.html#pressurepath-checks). Swainson's Warbler tend to stay low with small pressure variation such that a value of `0.5` is more adequate. In addition, following several trial and error test, we found that scaling the log linear pooling function to `4 * log(n) / n` provides more satifying result.
