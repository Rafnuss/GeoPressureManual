...
In the advance tutorial, we will construct the same three output with a Great Reed Warbler (18LCX), but this time including light and wind data. The workflow is very similar and we will see how to run the same procedure much faster. 

![*<small>Great Reed Warbler. 17 May 2019. Baranđanske slatine, Serbia. <a href="https://ebird.org/checklist/S56389245#eurwar1">eBird S56389245</a> | <a href="https://macaulaylibrary.org/asset/159594261">ML 159594261</a></small>*](assets/159594261.png){width=100%}

The first chapter is focused on constructing a likelihood map from light data. This step is optional in the workflow, and typically does not drastically change the result, but can be helpful to increase the computational efficiency during the creation of the graph. 

Here, we use an approach based on the threshold method and using a calibration of zenith angle (rather than time of sunrise). This approach is presented in more details in [section 2.4 of Nussbaumer et al. (2023a)](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.14043#mee314043-sec-0008-title)

A more thorough introduction to geolocation with light data can be found in <https://geolocationmanual.vogelwarte.ch/> (Lisovski et al., [2020](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/1365-2656.13036)). Note that other methods producing likelihood maps could also be used such as the ones presented in Basson et al. ([2016](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12555)) or Bindoff et al. ([2018](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12953)).
...
```{r 10-geopressure, cache = TRUE, message=FALSE, results='hide'}
tag <- tag_create("18LX",
...
Twilight outliar can be picked out visually when they don't follow a smooth line. 
...
**Do not over-edit the calibration period!**. The variability of twilight is important to build a calibration which adequately capture the range of uncertainty of a twilight. As it is easier to pick out outliar during long stationary periods than short ones, there is a risk of having more variability during short stationary periods and thus biases in the estimated position.
:::
...
```{r 10-geolight-map, message=FALSE, results='hide'}
...
It is worth checking how the likelihood map of light and pressure compare before building the graph. They should always overlap and if not, the tag and/or twilight labeling needs to be adjusted.
