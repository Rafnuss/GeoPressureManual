...
```{r, eval=F}
req <- list()
for (i_s in seq_len(nrow(pam$sta) - 1)) {
  ...
  req[[i_s]] <- wf_request(user = cds_user, request = request, transfer = F)
}
```

We suggest to save the wind data file in `data/5_wind_graph/`

```{r}
dir_save <- "data/5_wind_graph/18LX"
...
for (i_s in seq(1, length(req))) {
  filename <- paste0("18LX_", i_s, ".nc")
  wf_transfer(url = basename(req[[i_s]]$get_url()), service = "cds", user = cds_user, path = dir_save, filename = filename)
}
```
...
grl <- graph_add_wind(grl, pressure = pam$pressure, filename = "data/5_wind_graph/18LX/18LX_", thr_as = 100)
```
...
We first search the morphological information of the Great Reed Warbler using the [AVONET database](https://doi.org/10.6084/m9.figshare.16586228.v5). You can also overwrite any of these value if you know them. See [`flight_bird()`](https://raphaelnussbaumer.com/GeoPressureR/reference/flight_bird.html) for more details.

```{r}
bird <- flight_bird("Acrocephalus arundinaceus")
```

Using the bird created, we can convert a airspeed into a probability using the power method in the function [`flight_prob()`](https://raphaelnussbaumer.com/GeoPressureR/reference/flight_prob.html). See [Basic graph] for more information on [`flight_prob()`](https://raphaelnussbaumer.com/GeoPressureR/reference/flight_prob.html).
...
## Output 2: Marginal proability map
...
We can also visualize the shortest path and marginal map in GeoPressureViz. 
