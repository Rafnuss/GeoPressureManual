...
Moment d'émotion: this figure was the one that made me realise the potential of pressure to determine the position! The accuracy of the reanalysis data and the precision of the tag was such that position estimation from pressure should offer a good complementarity with light data! 
...
Note that if a position of the path is over water, it will be moved to the closest point onshore.
...
The `include_flight` is particularly useful here as it can help you compute altitude during the flight.

To extract the altitude for further analysis, I suggest useing the output of `pressurepath2altitude()` which convinently provide a single altitude estimate per time. Indeed, altitude is computed based on a single position reference (i.e., the positions of the path). However during a flight, the bird is actually moving. `include_flight` control which reference stationary period `stap_ref` is used and in general, both the previous and next stap_id are used.  `pressurepath2altitude()` will combine the estimate the altitude estimate from the two stap_id to return the most accurate flight alitutude.


<div class="alert alert-info" role="alert">
<h4 class="alert-heading mt-2"><strong>Why using GeoPressureAPI for altitude?</strong></h4>
