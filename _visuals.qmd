...
stap <- merge(
  ...
  data.frame(
    ...
    alt_mean = sapply(split(pressurepath$altitude, pressurepath$stap_id), \(x) {
      round(mean(x), 1)
    }),
    alt_med = sapply(split(pressurepath$altitude, pressurepath$stap_id), \(x) {
      round(median(x), 1)
    }),
    alt_min = sapply(split(pressurepath$altitude, pressurepath$stap_id), \(x) {
      round(min(x), 1)
    }),
    alt_max = sapply(split(pressurepath$altitude, pressurepath$stap_id), \(x) {
      round(max(x), 1)
    })
  )
)
...
write.csv(
  stap,
  glue::glue("./data/processed/{tag$param$id}/stap.csv"),
  row.names = FALSE
)
```
...
flight <- merge(
  ...
  data.frame(
    ...
    alt_mean = sapply(split(pp_f$altitude, pp_f$stap_s), \(x) {
      round(mean(x), 1)
    }),
    alt_med = sapply(split(pp_f$altitude, pp_f$stap_s), \(x) {
      round(median(x), 1)
    }),
...
write.csv(
  flight,
  glue::glue("./data/processed/{tag$param$id}/flight.csv"),
  row.names = FALSE
)
```
...
```{r visuals-export-params}
write(
  jsonlite::toJSON(graph$param, auto_unbox = TRUE, pretty = TRUE, force = TRUE),
...
frames <- frames_spatial(
  m,
  equidistant = TRUE,
  ext = raster::extent(tag$param$tag_set_map$extent)
) |>
...
```{r visuals-movevis-frame-preview} 
#| eval: false
plot(frames[[100]])
```
...
```{r visuals-movevis-animate} 
#| eval: false
animate_frames(
  frames,
  out_file = glue::glue("./assets/movevis_{tag$param$id}.gif"),
  height = 1000,
  width = 1000,
  overwrite = T
)
...
```{r visuals-ggmap-get-map} 
#| eval: false
istap_winter <- which(stap2duration(tag$stap) > 50)[2]

p0 <- get_googlemap(
  center = c(
    lon = path_most_likely$lon[istap_winter],
    lat = path_most_likely$lat[istap_winter]
  ),
  zoom = 9,
  maptype = "hybrid",
  size = c(1280, 1280),
...
p <- p0 +
  geom_contour(
    df,
    aes(x = x, y = y, z = layerP, color = after_stat(level)),
    linewidth = 2,
    breaks = c(.5, .1)
  ) +
