...
<div class="col">
Determining the positions, and over time trajectories, of wildlife is crucial to apprehend ecological relationships in nature. Since satellite devices (e.g. GPS) are too heavy for most bird species, lightweight geolocators remain an essential tool for ecologists. 
  
Geolocation by pressure provides a new and exciting opportunity to determine the position of tracked birds with high precision. 

As atmospheric pressure varies in space and time, a timeseries of pressure measurement recorded at a single location constitutes a unique signature which can be used for global positioning. 
...
The GeoPressure suite helps users tackle the challenges of geolocation by pressure and provides a toolkit to implement the two following methods published in the following two papers: 
...
GeoPressureR can currently read Swiss Ornithological Institute (SOI) files (`*.pressure`, `*.lux`, `*.acceleration`) and Migrate Technology files (`"*.deg"` and `"*.lux"`). 
...
Feel free to simply read through the manual, or to [fork the repository](https://github.com/Rafnuss/GeoPressureManual/fork) to run the examples yourself at your own pace.
